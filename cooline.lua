...
cooline:SetScript('OnEvent', function(self, event, ...)
	self[event]()
end)
...
function cooline.detect_cooldowns()
	...
	for id=1,total_spells do
		...
		local name = GetSpellBookItemName(id, BOOKTYPE_SPELL)
...
local function place_H(self, offset, just)
	self:SetPoint(just or 'CENTER', cooline, 'LEFT', offset, 0)
end
local function place_HR(self, offset, just)
	self:SetPoint(just or 'CENTER', cooline, 'LEFT', cooline_theme.width - offset, 0)
end
local function place_V(self, offset, just)
	self:SetPoint(just or 'CENTER', cooline, 'BOTTOM', 0, offset)
end
local function place_VR(self, offset, just)
	self:SetPoint(just or 'CENTER', cooline, 'BOTTOM', 0, cooline_theme.height - offset)
end
...
function cooline.VARIABLES_LOADED()
	...
	cooline:SetScript('OnDragStart', function(self)
		self.dragging = true
		self:StartMoving()
	end)
	cooline:SetScript('OnDragStop', function(self)
		self:on_drag_stop()
	end)
	cooline:SetScript('OnUpdate', function(self)
		self:EnableMouse(IsAltKeyDown())
		if not IsAltKeyDown() and self.dragging then
			self:on_drag_stop()
		end
		cooline.on_update()
	end)
	...
	cooline.overlay:SetFrameLevel(24) -- TODO self gets changed automatically later, to 9, find out why
